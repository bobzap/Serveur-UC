...
if ((paramsArray select 1) == 6) then{
	...
	if ((paramsArray select 1) == 7) then{
		IA_liberation_preset = 7;
	}
	else{
		if ((paramsArray select 1) == 8) then{
			IA_liberation_preset = 8;
		}//end if
		else{
			if ((paramsArray select 1) == 9) then{
				IA_liberation_preset = 9;
			}
		}
	}//end else
};
...
IA_liberation_arsenal = 0;

/* - Fuel consumption settings.
Time in minutes till a full tank depletes whilst the vehicle is standing still with a running engine.	*/
IA_liberation_fuel_neutral = 180;
// Time in minutes till a full tank depletes whilst the vehicle is driving below max speed.
IA_liberation_fuel_normal = 90;
// Time in minutes till a full tank depletes whilst the vehicle is driving at max speed.
IA_liberation_fuel_max = 45;

/* - Gameplay constant settings.
Name of the savegame namespace inside of the [ServerProfileName].vars.Arma3Profile file.	*/
GRLIB_save_key = "IA_LIBERATION_" + (toUpper worldName) + "_SAVEGAME";

IA_liberation_debug = false;																					// Enables debug messages in the server log
IA_liberation_savegame_debug = false;																			// Enables displaying of the whole save array in the server log on each save
...
GRLIB_secondary_missions_costs = [40, 30, 15];																	// Intel price for the secondary missions [FOB hunting, Convoy ambush, SAR].
GRLIB_secondary_objective_impact = 0.5;																			// The percentage impact against enemy combat readiness for a successful FOB hunt.
GRLIB_recycling_percentage = 0.5;																				// Percentage of resources you get back from recycling.
IA_liberation_production_interval = 30				/ GRLIB_resources_multiplier;								// Time in minutes until a production process is finished, when resources multiplier is set to 1.
...
IA_liberation_allowed_items_extension = [
...
box_transport_config = [
	...
	["rhsusf_M977A4_usarmy_wd", -8.5, [0.5,0.5,0.4], [0.5,-1.0,0.4], [0.5,-2.5,0.4], [0.5,-4.0,0.4], [-0.5,0.5,0.4], [-0.5,-1.0,0.4], [-0.5,-2.5,0.4], [-0.5,-4.0,0.4]],
	["rhsusf_M977A4_BKIT_M2_usarmy_wd", -8.5, [0.5,0.5,0.4], [0.5,-1.0,0.4], [0.5,-2.5,0.4], [0.5,-4.0,0.4], [-0.5,0.5,0.4], [-0.5,-1.0,0.4], [-0.5,-2.5,0.4], [-0.5,-4.0,0.4]],
	...
	["rhsusf_M977A4_BKIT_usarmy_d", -8.5, [0.5,0.5,0.4], [0.5,-1.0,0.4], [0.5,-2.5,0.4], [0.5,-4.0,0.4], [-0.5,0.5,0.4], [-0.5,-1.0,0.4], [-0.5,-2.5,0.4], [-0.5,-4.0,0.4]],
	["rhsusf_M977A4_BKIT_M2_usarmy_d", -8.5, [0.5,0.5,0.4], [0.5,-1.0,0.4], [0.5,-2.5,0.4], [0.5,-4.0,0.4], [-0.5,0.5,0.4], [-0.5,-1.0,0.4], [-0.5,-2.5,0.4], [-0.5,-4.0,0.4]],
	["rhsusf_M977A4_BKIT_usarmy_wd", -8.5, [0.5,0.5,0.4], [0.5,-1.0,0.4], [0.5,-2.5,0.4], [0.5,-4.0,0.4], [-0.5,0.5,0.4], [-0.5,-1.0,0.4], [-0.5,-2.5,0.4], [-0.5,-4.0,0.4]],
	["I_Heli_Transport_02_F", -6.5, [0,4.2,-1.45], [0,2.5,-1.45], [0,0.8,-1.45], [0,-0.9,-1.45]],
	["B_Heli_Transport_03_F", -7.5, [0,2.2,-1], [0,0.5,-1], [0,-1.2,-1]],
	["B_Heli_Transport_03_unarmed_F", -7.5, [0,2.2,-1], [0,0.5,-1], [0,-1.2,-1]],
	["rhsusf_CH53E_USMC", -7.5, [0,2.2,-1.4], [0,0.5,-1.4], [0,-1.2,-1.4]],
	...
	["swop_LAAT", -8.5, [0.5,2.5,1.0], [0.5,1.0,1.0], [0.5,-0.5,1.0], [0.5,2.5,2.0], [0.5,1.0,2.0], [0.5,-0.5,2.0]]
];
...
IA_liberation_large_storage_positions = [
	[-5.59961,3.60938,0.6],
	[-3.99902,3.60938,0.6],
	[-2.39941,3.60938,0.6],
...
IA_liberation_small_storage_positions = [
	[-2.34961,1.80078,0.6],
	[-0.75,1.80078,0.6],
...
IA_liberation_production_interval = ceil IA_liberation_production_interval;

if (IA_liberation_debug) then {private _text = format ["[IA LIBERATION] [DEBUG] config loaded for: %1", (name player)];_text remoteExec ["diag_log",2];};
