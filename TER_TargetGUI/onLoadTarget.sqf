...
_allTargets = (nearestObjects  [player, ["TargetBase"], 2500]) select {_x getVariable ["TER_isTarget", false]};
...
_lbTargets ctrlAddEventHandler ["LBSelChanged", {
	...
	_fncEHhit = {
		_this spawn {
			...
			sleep 0.01;
			_params = _target getVariable ["TER_LastHitPos", []];
			_params params [
				["_target",objNull],
				["_shooter",objNull],
				["_bullet",objNull],
	...
	// Get last hit parmaters:
	_lastHitPos = _currentTarget getVariable ["TER_LastHitPos",[]];
	if (count _lastHitPos > 0) then {
		//_lastHitPos params ["_target", "_shooter", "_bullet", "_position"];
		_shooter = _lastHitPos select 1;
		_position = _lastHitPos select 3;

		_cfgPath = configfile >> "CfgVehicles" >> typeOf _currentTarget;
		_displayName = _cfgPath call BIS_fnc_displayName;
		_t1 = localize "STR_TER_txtTarget" +" " +_displayName;
		_t2 = localize "STR_TER_txtShooter" +" "  +name _shooter;
		_t3 = localize "STR_TER_txtWeapon" +" " +getText (configfile >> "CfgWeapons" >> currentWeapon _shooter >> "displayName");
		_t4 = localize "STR_TER_txtDistance" +" " +str (_shooter distance _currentTarget) +"m";
		_magDetail = currentMagazineDetail  _shooter;
		_t6Pre =[_magDetail, 0,(_magDetail find "[") -1]  call BIS_fnc_trimString;
		_t6 = localize "STR_TER_txtMagazine" +" " +_t6Pre;
		_t5 = "";
		if (toLower typeOf _currentTarget find "acc" > -1) then {
			_t5 = localize "STR_TER_txtAccuracy" +" " +str ((1- (_position  distance (_currentTarget modelToWorld [-0.00390625,0.161621,-0.0233293])))*100) +"%";
		};
		_txt = [];
		{
			{_txt pushBack _x} forEach [_x, lineBreak];
		...
		_hitOffset = _currentTarget worldToModel _position;
		_offGUI = ctrlPosition _objModel vectorAdd [(_hitOffset select 0) /2, 0, (-(_hitOffset select 2) /2)];
		_objPointer ctrlSetPosition _offGUI;

		_pointer = "Sign_Sphere10cm_F" createVehicleLocal _position;
		_pointer attachTo [_currentTarget, _hitOffset];
		_pointer setObjectTexture [0,"#(argb,8,8,3)color(1,0,0,1.0,ca)"];
		_pointer spawn {
			sleep 10;
			deleteVehicle _this;
