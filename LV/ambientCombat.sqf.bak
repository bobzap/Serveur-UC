...
if(isNil("LV_AI_C_I_SGroups"))then{LV_AI_eastGroups = [];}; 
...
while{true}do{
	...
	if(count LV_ACS_activeGroups < _groupAmount)then{
		...
		_C_I_SRatio = floor((_sideRatios select 1) * _perRatio);
		_indeRatio = floor((_sideRatios select 2) * _perRatio);
		_lossRatio = _groupAmount - _westRatio - _C_I_SRatio - _indeRatio;
		...
		_C_I_SGroups = {(side _x) == east} count LV_ACS_activeGroups;
		_indeGroups = {(side _x) == resistance} count LV_ACS_activeGroups;
		//hint format["w: %1, e: %2, i: %3",_westGroups,_C_I_SGroups,_indeGroups];
		if(_westGroups < _westRatio)then{
			_side = 0;
		}else{
			if(_C_I_SGroups < _C_I_SRatio)then{
				_side = 1;
			}else{
