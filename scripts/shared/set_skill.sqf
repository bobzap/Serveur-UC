...
if ( _wounded ) then {
	...
	if ( _side == GRLIB_side_friendly || _side == GRLIB_side_enemy ) then {
		...
		if ( _inVehicle ) then {
			...
			_unit setSkill ["aimingshake", [ 0.6 * _skillmodifier ] call F_limitSkill];
		};
		_unit setSkill ["spottime", [ 0.7 * _skillmodifier ] call F_limitSkill];
		_unit setSkill ["spotdistance", [ 0.5 * _skillmodifier ] call F_limitSkill];
		_unit setSkill ["commanding", 0.7];
		_unit setSkill ["reloadSpeed", 0.7];
	} else {
		_unit setSkill ["aimingspeed", [ 0.6 * _skillmodifier ] call F_limitSkill];
		_unit setSkill ["aimingaccuracy", [ 0.6 * _skillmodifier ] call F_limitSkill];
		_unit setSkill ["aimingshake", [ 0.6 * _skillmodifier ] call F_limitSkill];
		_unit setSkill ["spottime", [ 0.6 * _skillmodifier ] call F_limitSkill];
		_unit setSkill ["spotdistance", [ 0.5 * _skillmodifier ] call F_limitSkill];
		_unit setSkill ["commanding", 0.7];
		_unit setSkill ["reloadSpeed", 0.5];
	};
